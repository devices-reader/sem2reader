...
procedure BoxGetEngGrpMon;
procedure ShowGetEngGrpMOn;

var
  mpEngGrpMon:  array[0..GROUPS-1,0..MONTHS-1,0..TARIFFS-1] of extended;
...
const
  quGetEngGrpMon: querys = (Action: acGetEngGrpMon; cwOut: 7+6; cwIn: 0; bNumber: $FF);

var
  bMon:   byte;

procedure QueryGetEngGrpMon;
begin
  InitPushCRC;
  Push(220);
  Push(bMon);
  if PushGrpMask then Query(quGetEngGrpMon);
end;
...
begin
  ...
  for Grp := 0 to GROUPS-1 do if GroupChecked(Grp) then   
    for Tar := 0 to TARIFFS-1 do mpEngGrpMon[Grp,bMon,Tar] := PopReals;
  ...
  if bMon <= ibMaxMon then 
    ...
    for Grp := 0 to GROUPS-1 do if GroupChecked(Grp) then begin
      ...
      for x := ibMinMon to ibMaxMon do begin
        e := 0;
        for Tar := 0 to TARIFFS-1 do e := e + mpEngGrpMon[Grp,x,Tar];
        s := s + Reals2StrR(e);
      end;
      AddInfo(s);
      ...
      if UseTariffs then begin
        for Tar := 0 to TARIFFS-1 do begin
          ...
          for x := ibMinMon to ibMaxMon do s := s + PackStrR(Reals2StrR(mpEngGrpMon[Grp,x,Tar]),GetColWidth);
          AddInfo(s);
        end;
      end;
    end;
    ...
    if frmMain.clbMain.Checked[Ord(acGetEngGrpHou)] then begin    
      ...
      for x := ibMinMon to ibMaxMon do begin
        ...
        for Grp := 0 to GROUPS-1 do if GroupChecked(Grp) then begin
          s := PackStrR('группа ' + IntToStr(Grp+1),GetColWidth);
            z := 0;
            for Tar := 0 to TARIFFS-1 do z := z + mpEngGrpMon[Grp,x,Tar];
