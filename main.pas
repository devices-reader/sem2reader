...
uses
  ...
  IdTCPClient, IdGlobal, VCLTee.TeEngine, VCLTee.Series, VCLTee.TeeProcs,
  VCLTee.Chart;
...
type
  TfrmMain = class(TfrmBasic)
    ...
    panBottom5: TPanel;
    panRight5: TPanel;
    btbClearChart: TBitBtn;
    btbSaveChart: TBitBtn;
    ...
    procedure stbMainDrawPanel(StatusBar: TStatusBar; Panel: TStatusPanel;
      const Rect: TRect);
    procedure btbClearChartClick(Sender: TObject);
    procedure btbSaveChartClick(Sender: TObject);
  private
...
uses support, kernel, soutput, sinput, ports, box, get_records, histograms;
...
begin
  AddTerminal(stT + '   // ' + FormatDateTime('hh:mm:ss:zzz dd.mm.yyyy',Now), clOut);
end;
...
procedure TfrmMain.btbClearChartClick(Sender: TObject);
begin
  inherited;
...
begin
  inherited;
  SaveLog(memInfo, 'Отчет ' + DateTime2Str + ' ');
  SaveLog(redTerminal, 'Терминал ' + DateTime2Str + ' ');
  ClearInfo;
end;

procedure TfrmMain.btbSaveChartClick(Sender: TObject);
begin
  inherited;
  with sd_ChartToFile do begin
    try
      InitialDir := ExtractFileDir(ParamStr(0));
      ...
      case FilterIndex of
        1: FileName := 'График ' + DateTime2Str + ' ' + '.bmp';
        2: FileName := 'График ' + DateTime2Str + ' ' + '.wmf';
      end;

      if Execute then begin
        case FilterIndex of
          1: crtMain.SaveToBitmapFile(FileName);
          2: crtMain.SaveToMetafile(FileName);
        end;
      end;
    except
      ErrBox('Ошибка при сохранении графика !')
    end;
  end;
end;
