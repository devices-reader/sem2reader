...
get_decret, get_start, get_bulk, get_engfrac1, get_engfrac2, get_errorlink,
...
get_current, get_current4, get_contacts3, get_calc1, get_calc2, get_stat1;
...
begin
  ...
  Items[Ord(acGetEngFrac1)]       := ('статистика обработки чисел в формате с плавающей запятой 1');
...
//    Items[Ord(acGetCalc2)]      := ('расчет №2');
    ...
    Items[Ord(acGetMemory0)]      := ('*чтение состояния памяти');
  ...
  with clbMain do  while (iwBox < Items.Count) do begin
    if Checked[iwBox] then begin
      case iwBox of
        ...
        Ord(acGetEngFrac1):    begin BoxGetEngFrac1;    Inc(iwBox); exit; end;
